...
void check_error(MYSQL * flag, string msg) { if(!flag) { //printf("%s", msg.c_str()); 
exit(0); } }
...
orders call_DB(string query) {
	...
	if(query.find("select") != -1 || query.find("SELECT") != -1){
		...
		close_DB(connect);
		if(row != NULL) 
{//orders(atoi(row[1]), string(row[2]), string(row[3]), string(row[4]), string(row[5]), string(row[6]), atof(row[7]), atof(row[8]), string(row[9]), string(row[10])).print();
