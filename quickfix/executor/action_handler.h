...
typedef struct orders {
	...
	orders(int t = 0, string coID = "", string ooID = "", string sm = "", string sd = "", string ty = "", double p = 0.00, double q = 0.00, string st="", string se = "") : 
		timestamp(t), client_orderID(coID), original_orderID(ooID), symbol(sm), side(sd), type(ty), price(p), quantity(q), status(st), sessionID(se){}

	inline orders operator- (const orders &op) { return orders(timestamp, client_orderID, original_orderID, symbol, side, type, price, quantity - op.quantity, status, sessionID); }
	void print() { 
		printf("timestamp = %d, clOrdID = %s, oOrdID = %s, symbol = %s, side = %s, type = %s price = %lf quantity = %lf status = %s sessionID = %s\n", timestamp, client_orderID.c_str(), original_orderID.c_str(), symbol.c_str(), side.c_str(), type.c_str(), price, quantity, status.c_str(), sessionID.c_str()); 
	}
}orders;
